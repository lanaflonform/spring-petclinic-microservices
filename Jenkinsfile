...
def deploy(tag, ns) {
  sh """
    ./linux-amd64/helm upgrade ${ns} helm/charts/spring-petclinic-microservices -f deployment-configs/${ns}/values.yaml --namespace=petclinic-${ns} --set image.tag=${tag} --set image.changeCause=jenkins-${BUILD_ID}
  """
}